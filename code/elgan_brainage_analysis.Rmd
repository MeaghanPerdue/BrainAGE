...
dat %>%
  group_by(sex) %>%
  summarise_at(vars(brainage_dif, brainage_dif_adj), list(mean = mean, sd = sd))
...
# Chronological age
plot.age_predage<-ggplot(aes(age, predicted_age), data=dat) + 
                      geom_point(aes(color=sex)) +
                      geom_smooth(method=lm, se=FALSE, size=.5, aes(color=sex)) +
                      geom_smooth(method=lm, se=FALSE, color="black") +
                      scale_x_continuous(limits=c(14.5,16.5), breaks=seq(14.5,17.5,.5)) +
                      scale_y_continuous(limits=c(13,25), breaks=seq(13,25,2)) +
...
plot.age_brainagedif<-ggplot(aes(age, brainage_dif), data=dat) + 
                      geom_point(aes(color=sex)) +
                      geom_smooth(method=lm, se=FALSE, size=.5, aes(color=sex)) +
                      geom_smooth(method=lm, se=FALSE, color="black") +
                      scale_x_continuous(limits=c(14.5,16.5), breaks=seq(14.5,17.5,.5)) +
